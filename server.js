...
// Online-la 'MONGODB_URI' environment variable irundha adha edukkum, illana local use pannum.
...
app.post("/register", async (req, res) => {
    try {
        ...
        res.status(400).json({ error: "User already exists! Vera name try pannunga." }); 
    }
});
...
app.post("/login", async (req, res) => {
    try {
        ...
        if (user && await bcrypt.compare(password, user.password)) {
            ...
            res.status(400).json({ error: "Invalid Login! Password check pannunga." }); 
        }
    } catch (e) {
